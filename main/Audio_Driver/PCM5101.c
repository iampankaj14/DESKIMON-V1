...
static esp_err_t bsp_i2s_write(void *audio_buffer, size_t len, size_t *bytes_written, uint32_t timeout_ms) {
    ...
    // Convert 0-100 Volume to 0.0 - 3.0 scaling factor (max 300% volume)
    float volume_factor = (Volume / 100.0f) * 3.0f;
...
void Volume_adjustment(uint8_t Vol) {
    ...
    ESP_LOGI(TAG, "Volume set to %d (scaling factor: %.2f)", Volume, (Volume / 100.0f) * 3.0f);
}
